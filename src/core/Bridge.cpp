...
void Bridge::startServe(const QString &source, int port, const QString &name, const QString &encoder, const QString &hw)
{
    std::string sSource = source.toStdString();
    std::string sName = name.toStdString();
    std::string sEnc = encoder.toStdString();
    std::string sHw = hw.toStdString();
    std::thread([this, sSource, port, sName, sEnc, sHw]()
                {
        ...
        if (!enc->initialize(params->width, params->height)) return;
        ...
        spdlog::info("Starting RTSP server and QML preview: {}", sSource);
        src->start(); })
        .detach();
}
